...
@section('content')
{!! BootForm::open()
    ->action(route('providers.update', $provider->uuid))
    ->put()
    ->id('provider-update')
    ->multipart()
!!}
{!! BootForm::bind($provider) !!}
<div class="card">
    <div class="card-header">
        <h4>{{__('general.edit')}} {{trans_choice('production.provider',1)}} <strong class="text-primary">{{ $provider->name }}</strong></h4>
    </div>
     <div class="card-body">
          ...
          <div class="col-md-4">
            ...
            <div class="form-group">
                <label class="checkbox my-10">
                   <input type="checkbox" name="geo_info" id="geo_info" {{ $provider->geo_info ? 'checked' : ''}} value=1 />
            ...
            <div class="form-group">
              <label>Logo</label>
               ...
               <img class="img-fluid mt-5" src="{{ asset( !is_null($provider->logo_path) ? 'storage/'.$provider->logo_path : 'assets/media/img/no_image.png') }}" />
             </div>
              ...
              <div class="card">
                ...
                <div class="card-body" id="json-options">
                      ...
                      <div class="row" id="box-options" >
                          ...
                          @forelse( $provider->options as $key => $value )
                              <div class="col-md-6 mb-5">
                                  <input type="text" class="form-control" name="option_keys[]" value="{{ $key }}" placeholder="sync_method, etc" />
                              </div>
                              <div class="col-md-6 mb-5">
                                  <input type="text" class="form-control"  name="option_values[]" value="{{ $value }}" placeholder="sync_method, etc"/>
                              </div>
                          @endforeach
                          <div class="col-md-6 mb-5">
